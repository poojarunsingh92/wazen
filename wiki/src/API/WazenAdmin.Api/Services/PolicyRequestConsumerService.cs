...
{
    public class PolicyRequestConsumerService : BackgroundService
    {
        ...
        public PolicyRequestConsumerService(IOptions<AzureServiceBusConnectionString> azureService, IServiceProvider serviceProvider)
        {
        ...
        private async Task ProcessMessagesAsync(Message message, CancellationToken token)
        {
            ...
            PolicyRequest policyRequest = JsonSerializer.Deserialize<PolicyRequest>(jsonString);
            Console.WriteLine($"Customer Received: ");
            Console.WriteLine(policyRequest);
            using (var scope = _serviceProvider.CreateScope())
            {
                try
                {
                    ...
                    var isExist = dbContext.Customers.Find(policyRequest.ID);
                    if (isExist != null)
                    {
                        //Update
                        var responseUpdate = dbContext.Update(policyRequest);
                        dbContext.SaveChanges();
                    }
                    else
                    {
                        //Add
                        var responseAdd = dbContext.Add(policyRequest);
                        dbContext.SaveChanges();
                    }
                }
                catch (Exception ex)
